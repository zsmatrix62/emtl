...
class EMTClient:
    ...
    def _get_captcha_code(self) -> tuple[float, Any]:
    ...
    def query_history_orders(self, size, start_time, end_time):
        """查询历史订单.
        ...
        :return dict:
        """
        resp = self._query_something("query_his_orders", {"qqhs": size, "dwc": "", "st": start_time, "et": end_time})
        if resp:
            return resp.json()

    def query_history_trades(self, size, start_time, end_time):
        """查询历史成交.
        ...
        :return dict:
        """
        resp = self._query_something("query_his_trades", {"qqhs": size, "dwc": "", "st": start_time, "et": end_time})
        if resp:
            return resp.json()

    def query_funds_flow(self, size, start_time, end_time):
        """查询资金.
        ...
        :return dict:
        """
        resp = self._query_something("query_funds_flow", {"qqhs": size, "dwc": "", "st": start_time, "et": end_time})
        if resp:
            return resp.json()

    def create_order(self, stock_code, trade_type, market: str, price: float, amount: int):
        """交易接口, 买入或卖出.
        ...
        resp = self._query_something("create_order", req_data=req_data)
        if resp:
            logger.info(resp.json())
            return resp.json()

    def cancel_order(self, order_str: str):
        """取消交易接口.
